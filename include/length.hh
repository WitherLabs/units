...
using inches         = impl::derived_kind<barleycorns, std::ratio<3,   1>>;
using hands          = impl::derived_kind<inches,      std::ratio<4,   1>>;
using yards          = impl::derived_kind<feet,        std::ratio<3,   1>>;
using chains         = impl::derived_kind<yards,       std::ratio<22,  1>>;
using furlongs       = impl::derived_kind<chains,      std::ratio<10,  1>>;
using miles          = impl::derived_kind<furlongs,    std::ratio<8,   1>>;
using leagues        = impl::derived_kind<miles,       std::ratio<3,   1>>;
using fathoms        = impl::derived_kind<metres,      std::ratio<463, 250>>;
using cables         = impl::derived_kind<fathoms,     std::ratio<100, 1>>;
using nautical_miles = impl::derived_kind<cables,      std::ratio<10,  1>>;
using links          = impl::derived_kind<feet,        std::ratio<66,  100>>;
using rods           = impl::derived_kind<links,       std::ratio<25,  1>>;
